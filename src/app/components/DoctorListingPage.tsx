...
export function DoctorListingPage() {
  ...
  const filteredDoctors = mockDoctors
    .filter((doctor) => {
      ...
      if (type && doctor.type !== type) return false;
  ...
  const handleBackClick = () => {
    // Go back to specializations with context
    const params = new URLSearchParams();
    if (type) params.append("type", type);
    if (hospitalId) params.append("hospitalId", hospitalId);
    navigate(`/specializations?${params.toString()}`);
  };
        ...
        <div className="max-w-4xl mx-auto mb-6 space-y-3">
          ...
          {type && !selectedHospital && (
            <div className="flex items-center gap-2 text-sm text-gray-600">
              ...
              >
                {type === "hospital" ? "Hospital" : "Private Clinic"}
              </Badge>
            </div>
          )}
        </div>
